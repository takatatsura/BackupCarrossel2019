...
/** Caracter de preenchimento dos limites para simplificar o tratamento das
 * imagens */
#define SENTINELA 255
/** GAP espaço que nao precisa ser da cor atual, mas que será considerado como
 * sendo (da cor atual) se seguido por outro pixel da cor atual
 */
#define GAP 4
...
#define TAM_X_CAMPO                                                            \
  (TAM_X_CAMPO_SEM_GOL + TAM_X_DO_GOL + TAM_X_DO_GOL) // em cemtimetros
#define TAM_Y_DO_GOL 40
#define CENTRO_X_GOL (TAM_X_CAMPO - TAM_X_DO_GOL)
#define CENTRO_Y_GOL TAM_Y_CAMPO / 2
#define POS_X_GOLEIRO                                                          \
  (TAM_X_DO_GOL + TAM_ROBO / 2 +                                               \
   2) // 2 cm a mais para o robo ter espaco para andar
...
#define NUM_IMGS_INTERVALO_CAMPO_VAZIO                                         \
  13 // a cada 13 imagens, uma sera considerada no campo vazio
 ...
 * robos
 */
#define N_IMAGENS_ATRASO 2

/** Numero de quadros entre o envio do comando ao robo e ele alcançar
 */
#define N_IMAGENS_INERCIA_ROBO 1

//#define SEM_PREVISAO

/** custo com alta penalidade indicando que nao existe
 * usado também -NAO_EXISTE, para agilizar comparacao <0, pode ser mais rapido
 * que !=NAO_EXISTE (especialmente em reais (float)
 */
...
// void calculaWaveFront(int objx, int objy, int objangulo, bool
// ignoraAdversario, Estado estado[NUM_ROBOS_TIME * 2 + 1], int mapa[170][130]);
void calculaCampoPotencial(int numRobo, int objx, int objy, int objAngulo,
