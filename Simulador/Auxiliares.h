...
/** Macro usada para verificação de rotinas, não deve ser usada durante o jogo
 */
#define DADO_NAO_OK                                                            \
  do {                                                                         \
    printf("Erro no teste (%s: %d)\n", __FILE__, __LINE__);                    \
    exit(3);                                                                   \
  } while (0)

#define VERIFICA_SE_DADO_OK(obtido, esperado)                                  \
...
inline void SCT_RGB(int ax, int bx, int lum, guint8 &r, guint8 &g, guint8 &b) {
  ...
  lum *=
      1.73; // 1.73 (ajuste para lum ser 442, soma vetorial de r, g e b = 255);
...
/** Inicia o vetor que conver RGB (4 bits de r, 4 de g e 4 de b) para a paleta
 * de cores calibradas */
void iniciaVetoresRGBPaleta(guint8 rgb_paleta[], int aInicio[], int bInicio[],
                            int lumInicio[], int aFinal[], int bFinal[],
                            int lumFinal[]);
...
inline float seno(float x) { return sin(x * M_PI / 180); }

inline float coss(float x) { return cos(x * M_PI / 180); }
...
public:
  ...
  // Valores ideais para o tamanho dos blobs. Estes valores mudam dependendo do
  // tamanho da imagem capturada pela camera
  ...
  Cor imgs[10][640 * 480]; // imagens para salvar quadros para analise e usado
                           // para calcular o campo vazio

  /** inicio das faixas do canal A, do B e do I(lum) */
  int aInicio[MAX_IND_CORES], bInicio[MAX_IND_CORES], lumInicio[MAX_IND_CORES];
...
/** Calcula a velocidade de cada roda a partir da imagem capturada do campo
 */
