...
struct Elem {
  ...
  struct Conj {
    int n;
    float x, y;
    int propTamOtimo; // proporcao do tamanho ótimo quanto maior mais proximo do
                      // blob otimo */
  } e[MAX_ELEMS];
} elem[2][MAX_IND_CORES]; //[2] para as linhas pares [0] e para as linha impares
                          //[1]
...
void separaEtiquetas(guint8 *pImgDigi, guint8 rgb_paleta[], int indCamera) {
  ...
  while (*pVaiProcessar != SENTINELA) {
    ...
    while (*pIni == cor) {
      ...
      /** verifica se existem pixels apos (a direita) de pFin que tambem sao da
       * cor*/
      ...
      if (pFin <= pAux) { // nao tem pixel da cor a direita do final, vamos
                          // procurar a esquerda
        /** verifica se existem pixels antes (a esquerda) de pFin que tambem sao
         * da cor*/
      ...
      /** verifica se existem pixels antes (a esquerda) de pIni que tambem sao
       * da cor*/
      ...
      if (pIni >= pAux) { // nao tem pixel da cor a esquerda do inicio, vamos
                          // procurar a direita
        /** verifica se existem pixels depois (a direta) de pIni que tambem sao
         * da cor*/
    ...
    // o blob de uma cor acaba de ser encontrado conpletamente, os dados obtidos
    // serao analizados abaixo
    int dx = maxX - minX + 1, dy = maxY - minY + 1;
    // 300,147 209,182; 276,214
    //		if (cor == 0)
    //		if (dx >= 6 && minY > 150 && minX > 250)// para teste no
    //debuguer 			pIni++;
    if (minLimiteXBlob <= dx && dx <= maxLimiteXBlob) {
      if (minLimiteYBlob <= dy && dy <= maxLimiteYBlob &&
          numMinPixelsBlob <= nPelBlog && nPelBlog <= numMaxPixelsBlob) {
        ...
        if (ind < MAX_ELEMS) {
          ...
          // calculo da proporcao do tamanho ótimo (diferencas entre as medidas
          // e os valores ideais)
          dx -= tamIdealXBlob;
          dx *= dx;
          dy -= tamIdealYBlob;
          dy *= dy;
          elem[campoImpar][cor].e[ind].propTamOtimo =
              dx + dy + abs(nPelBlog - numIdealPixelsBlob);
          contElem++;
        }
      }
    }
 ...
 * o parametro campoImpar deve ser [0] para linhas pares e [1] para linhas
 * impares
 */
void encontraObjetos(int campo, PosXYAux posNossosRobos[], PosXY &posBola,
                     PosXY posAdversarios[], int indCamera) {
  int num, numTime;
  // Encontra os NUM_ROBOS_TIME*2 grupos de cor do nosso time com numero de
  // pixel mais proximos com o ideal
  ...
  /** Procura as possiveis combinacoes entre etiquetas auxiliares e etiqueta do
   * time. Para cada etiqueta aux (indEtiqAux) verifica todas as etiquetas do
   * time, procurando combinacoes possiveis. Apenas as melhores candidatas
   * indEtiqAux, dentre as encontradas, sao relacionada com uma etiqueta do
   * time. relAux[i][j].indAux indica o indice da etiqueta da j-ésima cor
   * auxilar que pode estar relacionada à i-ésima etiqueta do time.
   */
  for (int indEtiqAux = 0; indEtiqAux < NUM_CORES_AUX; indEtiqAux++) {
    ...
    for (int i = 0; i < numTime; i++) { // repete para as numTime (dobro das
                                        // etiquetas encontradas) primeiros
      for (int j = 0; j < numAux; j++) {
        int dx = cTime[i].x - cAux[j].x;
        int dy = cTime[i].y - cAux[j].y;
        int d = dx * dx + dy * dy; // calcula a distancia, deixa ao quadrado
                                   // pois todas as distancias estao ao quadrado
   ...
   *  possibilidade 1: ordena relAux e cTime de acordo com o custo minimo
   * encontrado (distancia entre etiquetas) possibilidade 2: usar dados das
   * etiquetas encontradas anteriormente
   */

  //	cout << numTime << "|" << indNaoVazio << "]";

  /** agora numTime pode ser apenas o numero de robos; indNaoVazio é o numero de
   * etiquetas da cor do time com uma etiqueta de cor proxima */
  numTime = min(indNaoVazio, NUM_ROBOS_TIME);

  /** combina todos as etiquetas do time com as possiveis etiquetas auxiliares
   * relacionadas, otimizando para a melhor relacao
   */
  ...
  for (int i = 0; i < MAX_CONT_COMB; i++) {
    ...
    for (int j = 0; j < numTime; j++) {
      ...
      if (custo >= melhorCusto) // se o custo que esta sendo calculado já é
                                // maior, tenta outra combinacao
        break;
    }
  ...
  for (int j = 0; j < numTime; j++) {
    ...
    if (relAux[j][comb[indMelhorCusto][j]].prop < NAO_EXISTE) {
      ...
      posNossosRobos[comb[indMelhorCusto][j]].xAux =
          elem[campo][comb[indMelhorCusto][j] + COR_AUX1]
              .e[relAux[j][comb[indMelhorCusto][j]].indAux]
              .x;
  ...
  // Encontra os NUM_ROBOS_TIME grupos de cor do time adversario com numero de
  // pixel mais proximos do ideal
  num = elem[campo][COR_CONTRA].num;
  int numAdve = min(num, NUM_ROBOS_TIME);
  cTime = elem[campo][COR_CONTRA].e;
  for (int i = 0; i < numAdve; i++) {
    for (int j = i + 1; j < num;
         j++) { // repete para todos os elementos desta cor
      if (cTime[j].propTamOtimo < cTime[i].propTamOtimo) {
        Elem::Conj tmp = cTime[j];
        cTime[j] = cTime[i];
        cTime[i] = tmp;
  ...
  for (int i = 0; i < NUM_ROBOS_TIME; i++) {
    if (posNossosRobos[i].x != NAO_EXISTE) {
      if (nossoLado == ESQUERDA) {
        posNossosRobos[i].x =
            TAM_X_DO_GOL +
            (posNossosRobos[i].x - limitesCampo.minXSemGol) * fatorX;
        posNossosRobos[i].y =
            TAM_Y_CAMPO - (posNossosRobos[i].y - limitesCampo.minY) * fatorY;
        posNossosRobos[i].xAux =
            TAM_X_DO_GOL +
            (posNossosRobos[i].xAux - limitesCampo.minXSemGol) * fatorX;
        posNossosRobos[i].yAux =
            TAM_Y_CAMPO - (posNossosRobos[i].yAux - limitesCampo.minY) * fatorY;
      } else {
        posNossosRobos[i].x =
            TAM_X_DO_GOL + TAM_X_CAMPO_SEM_GOL -
            (posNossosRobos[i].x - limitesCampo.minXSemGol) * fatorX;
        posNossosRobos[i].y =
  ...
  for (int i = 0; i < NUM_ROBOS_TIME; i++) {
    if (posAdversarios[i].x != NAO_EXISTE) {
      if (nossoLado == ESQUERDA) {
        posAdversarios[i].x =
            TAM_X_DO_GOL +
            (posAdversarios[i].x - limitesCampo.minXSemGol) * fatorX;
        posAdversarios[i].y =
            TAM_Y_CAMPO - (posAdversarios[i].y - limitesCampo.minY) * fatorY;
      } else {
        posAdversarios[i].x =
            TAM_X_DO_GOL + TAM_X_CAMPO_SEM_GOL -
            (posAdversarios[i].x - limitesCampo.minXSemGol) * fatorX;
...
void combinaAdvesariosEntreCamposEAnterior(
  ...
  for (int i = 0; i < MAX_CONT_COMB; i++) {
    ...
    for (int j = 0; j < NUM_ROBOS_TIME; j++) {
      /* NUM_ROBOS_TIME + 1 é a posicao do primeiro dos adversarios apos rodos
       * do time [0..NUM_ROBOS_TIME-1] e bola [NUM_ROBOS_TIME] no estadoAnt */
      int ji = j + +NUM_ROBOS_TIME + 1;
      int k = comb[i][j];
      if (posAdversariosCampo1[k].x == NAO_EXISTE ||
          estadoAnt[ji].x == NAO_EXISTE)
        desloc *= 100; // penalidade, mas nao descarta pois o resto da
                       // combinacao pode ser a melhor
      else
        desloc *= abs(posAdversariosCampo1[k].x - estadoAnt[ji].x) +
                  abs(posAdversariosCampo1[k].y - estadoAnt[ji].y) + 1;
      if (desloc >= menorDesloc1) // se o custo que esta sendo calculado já é
                                  // maior, tenta outra combinacao
        break;
    }
  ...
  for (int i = 0; i < MAX_CONT_COMB; i++) {
    ...
    for (int j = 0; j < NUM_ROBOS_TIME; j++) {
      ...
      if (desloc >= menorDesloc2) // se o custo que esta sendo calculado já é
                                  // maior, tenta outra combinacao
        break;
    }
  ...
  //		cout<<endl;
  //		for (int j = 0; j < NUM_ROBOS_TIME; j++) {
  //			cout << "("<< posAdversariosCampo1[j].x<<","<<
  //posAdversariosCampo1[j].y<<")" ;
  //		}

  //	cout<<endl;
  //	for (int j = 0; j < NUM_ROBOS_TIME; j++) {
  //		cout << "("<< posAdversariosCampo2[j].x<<","<<
  //posAdversariosCampo2[j].y<<")" ;
  //	}
  ...
  for (int i = 0; i < NUM_ROBOS_TIME; i++) {
    if (posAdversariosCampo1[i].x != NAO_EXISTE &&
        (abs(posAdversariosCampo1[i].x - posAdversariosCampo2[i].x) >
         ...
         abs(posAdversariosCampo1[i].y - posAdversariosCampo2[i].y) >
             VELOCIDADE_MAXIMA / 60.0))
      posAdversariosCampo2[i].x = NAO_EXISTE;
  }
}
/*
void combinaAdvesariosEntreCampos(PosXY posAdversariosCampo1[NUM_ROBOS_TIME],
PosXY posAdversariosCampo2[NUM_ROBOS_TIME]) {
        * combina todos as etiquetas do time com as possiveis etiquetas
auxiliares relacionadas,
        ...
        for (int i = 0; i < MAX_CONT_COMB; i++) {
                ...
                for (int j = 0; j < NUM_ROBOS_TIME; j++) {
                        int k = comb[i][j];
                        if (posAdversariosCampo1[j].x == NAO_EXISTE ||
posAdversariosCampo2[k].x == NAO_EXISTE) desloc *= 100; // penalidade, mas nao
descarta pois o resto da combinacao pode ser a melhor else desloc *=
abs(posAdversariosCampo1[j].x - posAdversariosCampo2[k].x) +
abs(posAdversariosCampo1[j].y - posAdversariosCampo2[k].y) + 1; if (desloc >=
menorDesloc) // se o custo que esta sendo calculado já é maior, tenta outra
combinacao break;
                }
...
posAdversariosCampo2[j].y<<")" ;
        ...
        for (int j = 0; j < NUM_ROBOS_TIME; j++) {
                posAdversarios[j] =
posAdversariosCampo2[comb[indMenorDesloc][j]];
        }
        for (int j = 0; j < NUM_ROBOS_TIME; j++) {
                posAdversariosCampo2[j] = posAdversarios[j];
        }
        for (int i = 0; i < NUM_ROBOS_TIME; i++) {
                if (abs(posAdversariosCampo1[i].x - posAdversariosCampo2[i].x) >
VELOCIDADE_MAXIMA / 60.0 || abs(posAdversariosCampo1[i].y -
posAdversariosCampo2[i].y) > VELOCIDADE_MAXIMA / 60.0) posAdversariosCampo2[i].x
= NAO_EXISTE;
        }
}
...
void posicoes(guint8 *pImgDigi, guint8 rgb_paleta[], int indCamera) {
  ...
  encontraObjetos(0, posNossosRobosCampo1, posBolaCampo1, posAdversariosCampo1,
                  indCamera); // 0 indica linhas impares
  //
  //	for (int i = 0; i < NUM_ROBOS_TIME; i++)
  ...
  //")\n"; 	for (int i = 0; i < NUM_ROBOS_TIME; i++) 		cout << "adve (" <<
  //posAdversariosCampo1[i].x << ","
  //				<< posAdversariosCampo1[i].y << ") \t ";
  //
  //	cout << endl;

  //	cout << "ppppppppppppppppppppppppppppppppppp" << endl;
  encontraObjetos(1, posNossosRobosCampo2, posBolaCampo2, posAdversariosCampo2,
                  indCamera); // 0 indica linhas pares
  //
  //	for (int i = 0; i < NUM_ROBOS_TIME; i++)
  ...
  for (int i = 0; i < NUM_ROBOS_TIME; i++) {
    ...
    if (posNossosRobosCampo1[i].x != NAO_EXISTE) {
      anguloCampo1 =
          atang2(posNossosRobosCampo1[i].y - posNossosRobosCampo1[i].yAux,
      ...
      if (posNossosRobosCampo2[i].x != NAO_EXISTE) {
        anguloCampo2 =
            atang2(posNossosRobosCampo2[i].y - posNossosRobosCampo2[i].yAux,
                   posNossosRobosCampo2[i].x - posNossosRobosCampo2[i].xAux) -
            45;
        if (anguloCampo2 < 0)
          anguloCampo2 += 360;
        //				if (i==0)
        ...
        if (dAng > 180)
          dAng %= 180;
        if (dAng < 12) { // 32 graus é a velocidade máxima de rotacao do robo,
                         // mas nunca será usada
          ...
          if (difAngulosAbs(angAnt, anguloCampo1) >
            ...
            estado[i].dAngulo =
                difAngulos(anguloCampo1, angAnt) *
        ...
        /* media ponderada, 0,75 campo 2 + 0,25 campo 1 */
        float xCampo1 =
            (posNossosRobosCampo1[i].x + posNossosRobosCampo1[i].xAux) / 2;
        float xCampo2 =
            (posNossosRobosCampo2[i].x + posNossosRobosCampo2[i].xAux) / 2;
        estado[i].x = (xCampo2 * 3 + xCampo1) / 4;
        estado[i].dx = (xCampo2 - xCampo1) * 2; //*2 para colocar em 1/30 s
        float yCampo1 =
            (posNossosRobosCampo1[i].y + posNossosRobosCampo1[i].yAux) / 2;
        float yCampo2 =
            (posNossosRobosCampo2[i].y + posNossosRobosCampo2[i].yAux) / 2;
  ...
  if (posBolaCampo1.x !=
      NAO_EXISTE) { // a bola pode ser vista no campo 1 da imagem?
    if (posBolaCampo2.x !=
        NAO_EXISTE) { // a bola pode ser vista no campo 2 da imagem?
      estado[IND_BOLA].x = (posBolaCampo2.x * 3 + posBolaCampo1.x) / 4;
      estado[IND_BOLA].y = (posBolaCampo2.y * 3 + posBolaCampo1.y) / 4;
      estado[IND_BOLA].dx = (estadoAnt[IND_BOLA].x - posBolaCampo1.x) *
                            2; // 2* para transformas de 1/60 para 1/30 s
      estado[IND_BOLA].dy = (estadoAnt[IND_BOLA].y - posBolaCampo1.y) * 2;
      estado[IND_BOLA].angulo =
          atang2(estado[IND_BOLA].dy, estado[IND_BOLA].dx);
      estado[IND_BOLA].dAngulo =
          difAngulos(estado[IND_BOLA].angulo, estadoAnt[IND_BOLA].angulo);
    ...
    if (posBolaCampo2.x != NAO_EXISTE) { // mas pode ser vista no campo 2
      ...
      estado[IND_BOLA].dx =
          (estadoAnt[IND_BOLA].x -
           posBolaCampo2.x); // 2* para transformas de 1/60 para 1/30 s
  ...
  for (int i = IND_BOLA + 1; i < NUM_ROBOS_TIME * 2 + 1; i++, j++) {
    if (posAdversariosCampo1[j].x !=
        NAO_EXISTE) { // o adversario i pode ser vista no campo 1 da imagem?
      if (posAdversariosCampo2[j].x !=
          NAO_EXISTE) { // o adversario i pode ser vista no campo 2 da imagem?
        estado[i].x =
            (posAdversariosCampo2[j].x * 3 + posAdversariosCampo1[j].x) / 4;
        estado[i].y =
            (posAdversariosCampo2[j].y * 3 + posAdversariosCampo1[j].y) / 4;
        estado[i].dx = (posAdversariosCampo2[j].x - posAdversariosCampo1[j].x) *
                       2; // 2* para transformas de 1/60 para 1/30 s
        estado[i].dy =
            (posAdversariosCampo2[j].y - posAdversariosCampo1[j].y) * 2;
        estado[i].angulo = atang2(estado[i].dy, estado[i].dx);
        estado[i].dAngulo = difAngulos(estado[i].angulo, estadoAnt[i].angulo);
      } else { // o adversario i nao pode ser vista no campo 2 da imagem, só no
               // campo 1
        estado[i].x = posAdversariosCampo1[j].x;
        estado[i].y = posAdversariosCampo1[j].y;
        estado[i].dx = (estadoAnt[i].x - posAdversariosCampo1[j].x) *
                       2; // 2* para transformas de 1/60 para 1/30 s
      ...
      if (posAdversariosCampo2[j].x !=
          NAO_EXISTE) { // mas pode ser vista no campo 2
        estado[i].x = posAdversariosCampo2[j].x;
        estado[i].y = posAdversariosCampo2[j].y;
        estado[i].dx = (estadoAnt[i].x -
                        posAdversariosCampo2[j]
                            .x); // 2* para transformas de 1/60 para 1/30 s
        estado[i].dy = (estadoAnt[i].y - posAdversariosCampo2[j].y);
        estado[i].angulo = atang2(estado[i].dy, estado[i].dx);
        estado[i].dAngulo = difAngulos(estado[i].angulo, estadoAnt[i].angulo);
      } else { // o adversario i nao foi vista neta imagem (nem no campo 1 nem
               // no campo 2)
        estado[i].x = estadoAnt[i].x;
  ...
  if (!emJogo) {
    ...
    for (int i = 0; i < NUM_ROBOS_TIME * 2 + 1; i++) {
      printf("%3.0f, %3.0f, %3.0f, %3.0f, %3.0f|%3.0f, %3.0f, %3.0f, %3.0f, %3.0f\n",
             estado[i].x, estado[i].y, estado[i].dx,
             estado[i].dy, estado[i].angulo, estadoPrev[i].x,
