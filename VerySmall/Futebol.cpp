...
#define VIDEO // COMENTAR ESTA LINHA PARA PULAR A VERIFICAÇÃO DA CÂMERA
...
guint8 comb[MAX_CONT_COMB+1][NUM_ROBOS_TIME];
...
char seqImagens[MAX_SEQ][640 * 480 * 3];
int indSeqImagens = 0;

bool capturaSeqImagens = false, mostraSeqImagens = false;

Gtk::Button *btVolta, *btGrava, *btAvanca;
...
public:
...
#endif
	{
		...
		if (-1 == ioctl(getFD(), VIDIOC_S_CTRL, &ctrl))
			cout << "erro v4l2_control" << ctrl.value << "  " << errno << "  " << strerror(errno) << endl;
		else
	...
	virtual void video(const void * p) {
		if (p) {
			if (emJogo || !mostraSeqImagens) {
				yuyv_to_rgb24(640, 480, (unsigned char*) p, (unsigned char*) futCam[indCamera]->pImgCap);
				//memcpy(futCam[indCamera]->pImgCap, p, TAM_IMAGEM_640x480x3);
				if (capturaSeqImagens) {
					if (indSeqImagens>=0 && indSeqImagens < MAX_SEQ) {
						if (indSeqImagens == 80) {
							btGrava->modify_bg(Gtk::STATE_NORMAL, Gdk::Color("red"));
							btGrava->modify_bg(Gtk::STATE_PRELIGHT, Gdk::Color("red"));
//							btGrava->set_label("@@@@@@@@@@@@@@@@@@@@@@@@");
							printf("?");
							for (int pos1 = 0; pos1<100; pos1++){
								int base = pos1*640;
								for (int pos2 = 0; pos2<100; pos2++){
									futCam[indCamera]->pImgCap[base+pos2].r=255;
									futCam[indCamera]->pImgCap[base+pos2].g=255;
								}
							}
						} else {
							btGrava->unset_bg(Gtk::STATE_NORMAL);
							btGrava->unset_bg(Gtk::STATE_PRELIGHT);
							printf("!");
							for (int pos1 = 0; pos1<100; pos1++){
								int base = pos1*640;
								for (int pos2 = 0; pos2<100; pos2++){
						...
						memcpy(seqImagens[indSeqImagens++], futCam[indCamera]->pImgCap, TAM_IMAGEM_640x480x3);
					} else
						capturaSeqImagens = false;
				}
			} else {
				printf("%d\n", indSeqImagens);
				if (indSeqImagens>=0 && indSeqImagens < MAX_SEQ) {
				  memcpy(futCam[indCamera]->pImgCap, seqImagens[indSeqImagens], TAM_IMAGEM_640x480x3);
				}
			}
			if (!emJogo) {
#ifdef SALVA_ARQ
				...
				if (futCam[indCamera]->contImgsCampoVazio > 0) { // se valor maior que zero, captura imagens e calcula o campo vazio
...
public:
	Limites(int *hist, SelecionaCor *corSel) {
		histo = new Histograma(hist, 256, 30, corSel);
		;
		barrasCores = new BarraCores(256);
		this->hist = hist;
		vboxAux.add(*histo);
...
class JanPrincipal: public Gtk::Window {
	Gtk::Button *btSalva, *btCampoVazio, *btJogo, *btPenalidade, *btPosiciona, *btPartida;
	Gtk::VBox vBoxImgOrig, vBoxImgDigi, vBoxControlesCores, vBoxGeral;
	Gtk::HBox hBoxImgs, hBoxSelCor, hBoxCampoVazioLado, hBoxLado, hBoxBotoes, hBoxBotoesJogo, hBoxBotoesGrava;
	SelecionaCor *corSel;
...
public:
	JanPrincipal() {
		...
		imgOrig = new ImagemOriginal(320, 240, aHisto, bHisto, lumHisto);
		imgDig = new ImagemDigital(640, 480, aHisto, bHisto, lumHisto);
		nivelVazio = new Gtk::HScale(0, 200, 1);
		btSalva = new Gtk::Button("Salva");
		btCampoVazio = new Gtk::Button("Vazio");
		btJogo = new Gtk::Button("Jogo");
		tbtGraficos = new Gtk::ToggleButton("Graficos");
		btPenalidade = new Gtk::Button("Penalidade");
		btPosiciona = new Gtk::Button("Posiciona");
		btPartida = new Gtk::Button("Partida");
		rbEsquerda = new Gtk::RadioButton("Esquerda");
		rbDireita = new Gtk::RadioButton("Direita");
		corSel = new SelecionaCor(aHisto, bHisto, lumHisto);
		canalA = new Limites<HistogramaCanalA, BarraCoresCanalA>(aHisto, corSel);
		canalB = new Limites<HistogramaCanalB, BarraCoresCanalB>(bHisto, corSel);
		canalI = new Limites<HistogramaCanalI, BarraCoresCanalI>(lumHisto, corSel);

		btVolta = new Gtk::Button("<");
		btGrava = new Gtk::Button("o");
		btAvanca = new Gtk::Button(">");
		...
		hBoxBotoesGrava.add(*btVolta);
		hBoxBotoesGrava.add(*btGrava);
		hBoxBotoesGrava.add(*btAvanca);

		hBoxImgs.add(vBoxImgDigi);

		vBoxImgDigi.add(*imgDig);
		vBoxImgDigi.add(hBoxBotoesGrava);
		...
		btPartida->signal_clicked().connect(sigc::mem_fun(*this, &JanPrincipal::btPreparaPartida));
		tbtGraficos->signal_clicked().connect(sigc::mem_fun(*this, &JanPrincipal::tbtApresentaGraficos));

		btVolta->signal_clicked().connect(sigc::mem_fun(*this, &JanPrincipal::btGravacaoVolta));
		btGrava->signal_clicked().connect(sigc::mem_fun(*this, &JanPrincipal::btGravacaoGrava));
		btAvanca->signal_clicked().connect(sigc::mem_fun(*this, &JanPrincipal::btGravacaoAvanca));
	...
	void btGravacaoVolta() {
		...
		mostraSeqImagens = true;
	}

	void btGravacaoGrava() {
		//if (emJogo) return;
		capturaSeqImagens = true;
		emJogo = true;
		indSeqImagens = 0;
	}
	void btGravacaoAvanca() {
		//if (emJogo) return;
		if (indSeqImagens < MAX_SEQ + 1)
			indSeqImagens++;
		if (capturaSeqImagens) {
			capturaSeqImagens = false;
		}
...
static gboolean idle(gpointer data) {
	...
	if (cap == NULL) {
		...
		cap = new CapVideo("/dev/video2", 0, (JanPrincipal*) data, 0);
		// CapVideo(char * devName, int videoInput, Gtk::Window *jan, int indCamera)
	}
#ifdef VIDEO
	cap->queryFrames();
