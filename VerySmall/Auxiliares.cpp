...
void Props::put(string chave, string valor) { (*this)[chave] = valor; }
...
void geraMatrizComb(guint8 comb[MAX_CONT_COMB][NUM_ROBOS_TIME]) {
  ...
  continue1:
    ...
    //cout << "\n-> " << indsComb[0] << " " << indsComb[1] << " " << indsComb[2];
    ...
    for (int i1 = 0; i1 < NUM_ROBOS_TIME; i1++) {
      for (int i2 = i1 + 1; i2 < NUM_ROBOS_TIME; i2++)
        if (indsComb[i1] == indsComb[i2])
          // existem etiquetas duplicadas na combinacao, calcula proxima
          // combinacao
          goto continue1;
    }
...
/** classe que repreesenta uma camera com todas as variaveis necessárias para
 obter as informacoes de uma imagem da camera. esta classe foi criada para
 simplificar a utilizacao de duas cameras  */
...
void FutebolCamera::recalculaPixelsPosCentimetros() {
  tamIdealXBlob =
      4 * (limitesCampo.maxXSemGol - limitesCampo.minXSemGol) /
          TAM_X_CAMPO_SEM_GOL -
      1; // (4cm) ideal (-2 pois os limites (contorno) nao sao vistos
  tamIdealYBlob =
      4 * (limitesCampo.maxY - limitesCampo.minY) / TAM_Y_CAMPO - 1; // ideal;
  minLimiteXBlob = tamIdealXBlob / 2; // 50% do ideal
  minLimiteYBlob = tamIdealYBlob / 2;
  maxLimiteXBlob =
      tamIdealXBlob * 3; // 3 vezes pois quando a etiqueta esta se movimentando
                         // o rastro aul]menta a etiqueta
  maxLimiteYBlob = tamIdealYBlob * 3;

  numIdealPixelsBlob = (tamIdealXBlob + tamIdealYBlob) / 4; // (media)/2
  numIdealPixelsBlob = numIdealPixelsBlob * numIdealPixelsBlob * 3.14 /
                       2; // (quadrado de R) * PI (/2 é pois apenas as linhas de
                          // um campo sao contadas por vez)
  numMinPixelsBlob =
      numIdealPixelsBlob / 4; // como a area é o quadrado, divide 2 ao quadrado)
  numMaxPixelsBlob = numIdealPixelsBlob * (3 * 3);
  distIdealEtiquetas = 4 * (limitesCampo.maxXSemGol - limitesCampo.minXSemGol) /
                       TAM_X_CAMPO_SEM_GOL;
  // considerando que apenas as partes mais internas dos circulos (etiquetas)
  // sao visiveis
  minDistEtiquetas = distIdealEtiquetas / 2;
  // considerando que apenas as partes mais externas dos circulos (etiquetas)
  // sao visiveis
...
void FutebolCamera::converteRGB24Paleta(guint8 *pImgDigi, Cor *pImgCap,
  ...
  memset(pImgDigi + posLinFinal, SENTINELA, TAM_IMAGEM_640x480x1 - posLinFinal);
  ...
  int linInicioGol =
      linInicio +
      (linFinal - linInicio) * (TAM_Y_CAMPO - TAM_Y_DO_GOL) / 2 / TAM_Y_CAMPO -
      15;
  int linFinalGol =
      linInicioGol + (linFinal - linInicio) * TAM_Y_DO_GOL / TAM_Y_CAMPO + 15;
...
void FutebolCamera::processa() {
  ...
  if (!emJogo) { //	restaura a imagem com cores tranformadas, como antes do
                 //processamento da segmentacao
...
void FutebolCamera::leParametros() {
  ...
  limitesCampo.minXSemGol =
      props->getInt(strIndCam + "limitesCampo.minXSemGol", 10);
  limitesCampo.maxXSemGol =
      props->getInt(strIndCam + "limitesCampo.maxXSemGol", 620);
...
void FutebolCamera::leParametros(int inicio[], int final[], string canal) {
  ...
  for (int i = 0; i < MAX_IND_CORES; i++) {
    inicio[i] =
        props->getInt(strIndCam + "InicioFaixaCanal" + canal + "Cor" + conv[i]);
    final[i] =
        props->getInt(strIndCam + "FinalFaixaCanal" + canal + "Cor" + conv[i]);
  }
...
void FutebolCamera::salvaImgCampoVazio() {
  ...
  memcpy(pImgAux, pImgCap, TAM_IMAGEM_640x480x3); // necessario somente para
                                                  // desenvolvimento (sem video)
  memcpy(pImgCap, pCampoVazio, TAM_IMAGEM_640x480x3);
  imgCap->save(strIndCam, "png");
  memcpy(pImgCap, pImgAux, TAM_IMAGEM_640x480x3); // necessario somente para
                                                  // desenvolvimento (sem video)
}
...
FutebolCamera::~FutebolCamera() { delete props; }
